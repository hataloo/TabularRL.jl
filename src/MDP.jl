...
# struct MDPController{MDPType, StateType}
#     mdp::MDPType
#     state::StateType
#     terminated::Bool
#     MDPController(mdp::T)
...
mutable struct TabularMDP{R <: Distribution} <: AbstractTabularMDP{R}
    ...
    terminated::Bool # If current episode has terminated - must call reset! before next step.
    ...
    TabularMDP(S::Vector{Int64}, A::Vector{Int64}, P::Array{Float64,3}, 
        ...
        new{Dirac{Float64}}(S, A, P, P_dist, Dirac.(R), DiscreteNonParametric(S, μ), γ, terminal, -1, true)
    end
    ...
    TabularMDP(S::Vector{Int64}, A::Vector{Int64}, P::Array{Float64,3}, 
        ...
        new{T}(S, A, P, P_dist, R, DiscreteNonParametric(S,μ), γ, terminal, -1, true)
    end
    TabularMDP(P::Array{Float64,3}, 
            R::Union{Array{T,2},Array{T,3}}, μ::Vector{Float64}, γ::Float64) where T <: Distribution = begin
        numberOfStates, numberOfActions = length(μ), size(P,3)
...
function reset(mdp::TabularMDP)
    mdp.s = sampleInitialState(mdp)
    mdp.terminated = mdp.terminal[mdp.s]
    return mdp.s, mdp.terminated
end
...
function step(mdp::TabularMDP, a::Int64)
    if mdp.terminated throw(InvalidStateException("MDP has terminated, must call reset(mdp) before step.", :terminated)) end
    s_new = sampleState(mdp, mdp.s, a)
    if ndims(mdp.R) == 2
        r = sampleReward(mdp, mdp.s, a)
    else
        r = sampleReward(mdp, s_new, mdp.s, a)    
    end
    mdp.terminated = mdp.terminal[s_new]
    mdp.s = s_new
    return s_new, r, mdp.terminated
end
